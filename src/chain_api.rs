...
/// Interface for ontio_write_wasmvm_memory
#[no_mangle]
pub unsafe extern "C" fn ontio_write_wasmvm_memory(
    vmctx: *mut VMContext,
    buffer: Memptr,
    data_ptr: u32,
    data_len: u32,
    ...
    let res = catch_wasm_panic(|| {
        ...
        if memory.len() < (data_ptr + data_len) as usize {
            panic!("ontio_write_wasmvm_memory out of bound");
        }
        let buff = std::slice::from_raw_parts(buffer, data_len as usize);
